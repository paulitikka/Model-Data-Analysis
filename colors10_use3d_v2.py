...
#%
#d_som['tipend']=pd.DataFrame(np.zeros((len(d_som),1)))
#c1=d_som['sample']==[4]
#c2=d_som['sample']==[11]
#c3=d_som['sample']==[16]
#ctot=c1+c2+c3
#d_som=d_som[ctot]
##%
#d_tip_x=np.mean(d_som[d_som['time']==1]['x']) #210.0200810810811
#d_tip_y=np.mean(d_som[d_som['time']==1]['y']) #167.06678918918925
d_tip_x=175
d_tip_y=300

#d_corner_x=np.mean(d_som[d_som['time']==np.max(d_som['time'])]['x']) #231.52706827293287
#d_corner_y=np.mean(d_som[d_som['time']==np.max(d_som['time'])]['y']) #155.3436961899398
d_corner_x=250
d_corner_y=200
...
df_speed_ilya5b=df_speed_ilya5b.T*0.24
#%
...
d_som.to_csv('C:/python/SOM_REF_EXPS AND MODS/irist.csv', index=False,header=False)
#%%
#$.19 to microm per minute in this resuolution
...
#cornerr_s=np.var(d_som['corner'])/np.mean(d_som['corner'])
...
#plt.hist(d_som['time'][aa & bb & cd],500)
...
#%%
#d_x11=d_x1.loc[(d_x1 >=0.2) & (d_x1 <=0.9)]
#d_y11=d_y1.loc[(d_y1 >=0.1) & (d_y1 <=0.9)]
#%%
...
per_G2=len(dsom_G2_vel)/len(d_y1)
#0.08051744641355106
per_G8=len(dsom_G8_vel)/len(d_y2)
#0.1423913043478261
...
#,range=[0,80])
#plt.hist(d_som['y'].fillna(0), bins=20,range=[-1,1]) #or, check range
#plt.hist(d_som['y'].fillna(0), bins=10,range=[-10,650]) #or, check range; larger range less bins
...
#        dist=[]
#        T=[]
#        mt=np.zeros((len(ooz1),1))
#        mc=np.zeros((len(ooz1),1))
#        md=np.zeros((len(ooz1),1))
#        st=np.zeros((len(ooz1),1))
#        
#        for i in range(len(ooz1)):
#            x.append(np.array(d_som.ix[list(ooz1[i].index),'x']))
#            y.append(np.array(d_som.ix[list(ooz1[i].index),'y']))
#            z.append(np.array(d_som.ix[list(ooz1[i].index),'z']))
#            colors.append(np.array(d_som.ix[list(ooz1[i].index),'speed.xy']))
#            dist.append(np.array(d_som.ix[list(ooz1[i].index),'tipendDist']))
#            T.append(np.array(d_som.ix[list(ooz1[i].index),'time']))
#        
#        for i in range(len(ooz1)):
#            x[i] = abs(x[i]/max(x[i]))
#            y[i] = abs(y[i]/max(y[i]))
#            z[i] = abs(z[i]/max(z[i]))
#            colors[i] = abs(colors[i]/max(colors[i]))
#            dist[i] = abs(dist[i]/max(dist[i]))
#    #        T[i]=abs(T[i]/max(T[i]))  
#    
#            #%Rounding for plotting purposes
#        for i in range(len(ooz1)):
#            for j in range(len(ooz1[i])):
#                if x[i][j] >= 0.95:
#                    x[i][j]=0.95
#                if x[i][j] <= 0.05:
#                    x[i][j]=0.05
...
#        vector=[mt1,mc1,md1,st1]
#        #%
#        return vector
#       #%
#    vector1=ooz_to_tcds(ooz1)
#    vector2=ooz_to_tcds(ooz2)
#%    ooon2=[x,y,z,colors,dist,T]
...
def showx(d_som,ooon,ooz1,ooz2):
    ...
    on_index2=[]
    on_index3=[]
#    on_index=list(ooon.index)+list(ooz1.index)+list(ooz2.index)
...
#    https://stats.stackexchange.com/questions/70801/how-to-normalize-data-to-0-1-range
    ...
    T=nt
        ...
        #%
#    plt.figure()
#    plt.plot(d_som['time'],d_som['x'])
#    plt.xlabel('Time / AU')
#    plt.ylabel('X mean / AU')
#    plt.figure()
#    plt.plot(d_som['time'],d_som['y'])
#    plt.xlabel('Time / AU')
#    plt.ylabel('Y mean/ AU')
#    plt.figure()
#    plt.plot(d_som['time'],d_som['speed'])
#    plt.xlabel('Time / AU')
#    plt.ylabel('Speed mean / AU')
...
#    mt=mt1+vector1[0]+vector2[0]
#    mc=mc1+vector1[1]+vector2[1]
#    md=md1+vector1[2]+vector2[2]
#    st=st1+vector1[3]+vector2[3]
   ...
   'Group 7 Late','Group 8 Late',]
    ...
    #ax.set_xlim3d(0,1) #time is more than one!!
    #ax.set_ylim3d(0,1)
    #ax.set_zlim3d(0,1)
...
#    legend.get_frame().set_facecolor('w')
    ...
    #plt.set_zlabel('Distance normed/ AU')
    plt.title('SOM cluster groups')
#    legend = plt.legend(loc='lower center', shadow=True, fontsize='x-small',markerfirst=True,markerscale=0.3)
#    legend.get_frame().set_facecolor('w')
...
#                        fontsize='xx-small',markerfirst=False,markerscale=0.3)
#    legend.get_frame().set_facecolor('w')
...
#    for a in range(4):
#        for b in range(4):
#            sns.jointplot(x=td[(a+1)*(b+1)-1]["Distance normed / AU"], 
#                               y=td[(a+1)*(b+1)-1]["Speed normed / AU"], 
#                               kind='kde', color="skyblue",ax=axes[a,b])
...
#    fig.tight_layout()
#    pylab.savefig('XY locations.png')
...
#    https://matplotlib.org/gallery/images_contours_and_fields/contourf_demo.html#sphx-glr-gallery-images-contours-and-fields-contourf-demo-py
#    plt.colorbar(p).set_label('Cell amount', rotation=90)
...
#    plt.colorbar(p).set_label('Cell amount', rotation=90)
    ...
    fig7 = plt.figure()
    ax = plt.axes(projection='3d')
#ax.scatter3D(x, y, z, s=80,c=colors, cmap='terrain')
...
#    zn=2000*zn
    ...
    fig, ax = plt.subplots()
#    plt.axis([(min(x)-round(var(x)*4,2)), (max(x)+round(var(x)*4,2)), 
#              (min(y)-round(var(y)*4,2)), (max(y)+round(var(y)*4,2))])
...
#    https://stackoverflow.com/questions/40766909/suggestions-to-plot-overlapping-lines-in-matplotlib
    return result
#%%Test 16419, seismic is the blue to red 
#(via white.. coolwarm would be via mix, gist_ncar, rainbow)
...
#        eval(line.strip())    
#python folder is 'full' and may not save pictures, change of the folder is possibly needed
...
#%%Analysing the groups one-by-one
