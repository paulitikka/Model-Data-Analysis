...
#2D Presumably ok codes and routines, tikka 7.8.19
#%%
...
import itertools
import statistics
#%%(Importing raw files is below all)
import os
...
yesx2=[]
yesx3=[]
#C:\python\Model_results\PERUS\2d\Opti\field\Unif
#Speed_3D_ci(dataframes,cond,conda,time,dt,name)
#PERUS/2d/Opti/non-field/Unif
...
#dataframes,cond,cond1,conda,time,name
#%
resulti= Amount_3D_ci(dataframes,
...
#        dfcell_names3d.ix[:,2]=dfcell_names3d.index.unique()
        #%
        dfcell_names3d.ix[:,1]=list(np.where(dfcell_names3d.ix[:,1] > 1, 'ACells', 'NPCells'))
        dfcell_names3da=dfcell_names3d.ix[:,0:2] 
#        dfcell_names3dax=dfcell_names3da
        cols = dfcell_names3da.columns.tolist()
        cols = cols[-1:] + cols[:-1]
        #%
#        dfcell_names3dax.ix[:,1]=dfcell_names3da.ix[:,0]
#        dfcell_names3dax.ix[:,0]=dfcell_names3da.ix[:,1]
...
# https://stackoverflow.com/questions/522563/accessing-the-index-in-python-for-loops
    #%df_model
#    time=100
#    dfJEE=dataframes[9]  # this for correct indeces
...
#    elif mtt.shape != mmf.shape: 
    for i in range(len(mmf)):
        ...
        #%
#    if mt2.shape == mmf2.shape:
#        mtt=mmf2
#    elif mt2.shape != mmf2.shape: 
